 ...
 *
 * ## Human-readable rationale (SP2b.1 / SP2b.5)
 *
 * Every subtype exposes a [rationale] property (default `emptyList()`) carrying
 * zero or more human-readable strings that describe the rule evaluations behind
 * the decision — e.g. `["Slot available: 0 active of 2 max",
 * "Train 'T1' admitted (FIFO from unapproved queue)"]`.
 *
 * Rule-based dispatchers generated by [CandidatePathRuleEngine] populate this list
 * with one entry per rule that influenced the outcome.  LLM-backed [Dispatcher]
 * implementations may populate it with chain-of-thought or explanation text.
...
sealed class DispatchDecision {
	 ...
	 *
	 * An empty list (the default) means "no explicit rationale recorded" — the
	 * applier does not log anything for an empty list.  Non-empty lists are logged
	 * at DEBUG level alongside the applied decision for post-run analysis (SP2b.5).
	 *
	 * LLM-backed [Dispatcher] implementations may populate this with
	 * chain-of-thought or explanation text; [CandidatePathRuleEngine.selectWithRationale]
	 * produces one entry per rule that influenced the outcome.
	 *
	 * @since Issue #556 (SP2b.1 — Goal 10); changed to [List] in Issue #560 (SP2b.5)
	 */
	open val rationale: List<String> = emptyList()
	 ...
	 * @property rationale Rule-evaluation strings explaining why this train was
	 *   admitted (empty for dispatchers that do not record rationale).
	 */
	 ...
	 *   seconds.  Must be > 0.
	 * @property rationale Rule-evaluation strings explaining why this hold was
	 *   issued (empty for dispatchers that do not record rationale).
	 *
	 ...
	 * @property rationale Rule-evaluation strings explaining this switch command
	 *   (empty for dispatchers that do not record rationale).
	 *
	 * @since Issue #774 (SP1.7 — Goal 10 threading contract)
	 */
	 ...
	 * @property rationale Rule-evaluation strings explaining this release command
	 *   (empty for dispatchers that do not record rationale).
	 *
	 * @since Issue #774 (SP1.7 — Goal 10 threading contract)
	 */
	 ...
	 * @property rationale Rule-evaluation strings explaining this route request
	 *   (empty for dispatchers that do not record rationale).
	 *
	 * @since Issue #774 (SP1.7 — Goal 10 threading contract)
	 */
