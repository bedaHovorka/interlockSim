 ...
 * - Conservative approach: Tests focus on ShuntingLoop state and initialization (kDisco-safe)
 * - Full simulation execution requires kDisco framework integration
 *
 * Railway Context:
...
class ShuntingLoopOperationalTest : KoinTestBase() {
	...
	inner class LoopPathCoordinationTests {
		...
		fun `conflicting train waits for loop to clear`() {
			...
			// 4. Train is activated in kDisco simulation framework
	...
	inner class LoopTimingTests {
		 ...
		 * cars. Simulator must model this via kDisco hold() operations.
		 */
		@Test
