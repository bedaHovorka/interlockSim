...
/**
 * Unit tests for Multimap extension functions on sorted maps.
 *
 * Tests the Kotlin-idiomatic approach using sorted MutableMap<K, MutableSet<V>>
...
class MultimapExtensionsTest {
	private lateinit var multiMap: MutableMap<String, MutableSet<Int>>

	@BeforeTest
	fun setUp() {
		multiMap = mutableMapOf()
	}
...
// Use LinkedHashMap (inserted-order, not sorted) to ensure sort happens
		val linkedMap = linkedMapOf<Int, MutableSet<String>>()
...
// Verify specific key
	...
	fun differentValueTypes_strings_worksCorrectly() {
		val stringMap: MutableMap<String, MutableSet<String>> = mutableMapOf()
	...
	fun differentKeyTypes_integers_worksCorrectly() {
		val intKeyMap: MutableMap<Int, MutableSet<String>> = mutableMapOf()
	...
	fun putMulti_nullValue_storesNull() {
		val multiMapNullable: MutableMap<String, MutableSet<Int?>> = mutableMapOf()
