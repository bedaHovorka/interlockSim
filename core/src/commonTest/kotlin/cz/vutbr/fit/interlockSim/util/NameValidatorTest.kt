 ...
 *
 * All tests live flat in this one class on purpose: JUnit Jupiter's `@Nested` is a
 * jvmTest-only dependency and so is not available in commonTest, and discovery of
 * nested classes without `@Nested` is undocumented behaviour. A flat top-level class
 * is the only shape guaranteed to run on both the JVM and linuxX64.
 */
