 ...
 *
 * The test drives the rule-based shunting loop, watches for the first train that stands at a
...
class Issue1025CommittedTrainReleaseTest : DispatcherKoinTestBase() {
	...
	fun `releasing the next block of a train committed at a proceed aspect frees nothing and the train books it`() {
		val context: DefaultSimulationContext = TestFixtures.newShuntingSimulationContext().tracked()
		...
		// DynamicTrackBlock.enter fires OccupancySet through the environment's block-event bus.
		context.addBlockEventListener(
			BlockEventListener { event ->
				...
				if (event is BlockEvent.OccupancySet &&
					...
					// Listeners run on the simulation thread only, so first-writer-wins is race-free.
					if (bookedAfterInjection.get() == null) bookedAfterInjection.set(event.time)
				}
			}
		)

		// Polls the live state every 0.1 s of simulated time and strikes once, inside the window.
		val injector =
			object : Process() {
				override suspend fun actions() {
					while (injection.get() == null) {
						...
						val released = releaser.releaseUntravelledTail(trainId, listOf(blockId)).released
						injection.set(Injection(trainId, blockId, time(), released))
					}
				}
			}
		...
		// Before the fix this throws SimulationException[FATAL] "Wrong state: FREE , expected : RESERVED".
		context.run()

		val done = injection.get()
		assertThat(done, "injection (null means no train was caught at a proceed aspect: vacuous)").isNotNull()
		assertThat(done!!.released, "blocks freed while a proceed aspect stood at the boundary").isEmpty()
	...
	private companion object {
		...
		const val POLL_PERIOD_SECONDS = 0.1
	}
}
