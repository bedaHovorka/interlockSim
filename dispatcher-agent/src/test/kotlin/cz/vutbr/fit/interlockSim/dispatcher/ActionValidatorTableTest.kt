...
class ActionValidatorTableTest {
	companion object {
		...
		): QueuedTrainView = QueuedTrainView(trainId = trainId, destinationInOutName = destination, queuedSinceSimTime = 5.0)

		fun reservationView(
			trainId: String,
			target: String,
			blocks: List<String> = listOf("kA", "kB")
		): ReservationView =
		...
		): BlockView = BlockView(blockId = blockId, state = state, occupantTrainId = occupant)

		// ── Single-action test cases ───────────────────────────────────────────
		//
		// Map: RejectionCode → (action, observation) that triggers it via validate().

		val SINGLE_CASES: Map<RejectionCode, Pair<DispatchAction, DispatcherObservation>> =
			mapOf(
				// ── Shape / identity ───────────────────────────────────────────────
				...
				RejectionCode.TRAIN_ALREADY_ACTIVE to (
					DispatchAction.ApproveTrain("T1") to
						obs(
				...
				RejectionCode.TARGET_NOT_TRAIN_DESTINATION to (
					DispatchAction.RequestRoute("T1", "A", "C") to
						obs(
				...
				RejectionCode.ORIGIN_NOT_AT_TRAIN_POSITION to (
					DispatchAction.RequestRoute("T1", "sigB", "B") to
						obs(
							trains =
								listOf(
									trainView(
				...
				RejectionCode.NO_ROUTE_HELD to (
					DispatchAction.CancelRoute("T1") to
						obs(
							trains = listOf(trainView("T1", TrainPhase.RUNNING)),
		...
		val BATCH_CASES: Map<RejectionCode, Pair<List<DispatchAction>, DispatcherObservation>> =
			mapOf(
