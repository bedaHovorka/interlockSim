...
class AllBlockOccupanciesTool(
	...
	override suspend fun execute(args: Map<String, Any?>): ToolResult {
		...
		return runCatching { perceptionPort.allBlockOccupancies() }
			.fold({ ToolResult.Success(it) }, { ToolResult.Error("all_block_occupancies failed: ${it.message}", it) })
	}
}
