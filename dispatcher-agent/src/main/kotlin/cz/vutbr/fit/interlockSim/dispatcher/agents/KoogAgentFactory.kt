	...
	suspend fun createAgent(context: DefaultSimulationContext): KoogDispatchAgent {
		...
		val sinkHolder = SinkHolder(
			EmittedActionSink { action ->
				val decisions: List<cz.vutbr.fit.interlockSim.sim.DispatchDecision> = when (action) {
					is DispatchAction.ApproveTrain ->
						listOf(cz.vutbr.fit.interlockSim.sim.DispatchDecision.ApproveTrain(trainId = action.trainId))
					is DispatchAction.RequestRoute ->
						listOf(
							cz.vutbr.fit.interlockSim.sim.DispatchDecision.RequestRoute(
					...
					is DispatchAction.CancelRoute ->
						listOf(cz.vutbr.fit.interlockSim.sim.DispatchDecision.ReleaseRoute(trainName = action.trainId))
