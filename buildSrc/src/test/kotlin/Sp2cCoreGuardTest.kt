...
class Sp2cCoreGuardTest {
	...
	@Test
	fun `skips with a clear reason when repoRoot is not a git repository`(
		@TempDir tempDir: File,
	) {
		// No TestGitRepo(tempDir) — plain directory, as Docker build contexts are: .dockerignore
		// deliberately excludes .git, so the guard must not crash when there simply is no repo.
		val result = Sp2cCoreGuard.evaluate(tempDir, "deadbeef", allowlistedPaths = emptySet())

		val skipped = assertInstanceOf(Sp2cCoreGuard.Result.Skipped::class.java, result)
		assertTrue(skipped.reason.contains("git repository")) {
			"expected skip reason to mention the missing git repository, was: ${skipped.reason}"
		}
	}
