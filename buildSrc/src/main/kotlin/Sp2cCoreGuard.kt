...
object Sp2cCoreGuard {
	...
	fun evaluate(
		...
		val (workTreeExitCode, _) = runGit(repoRoot, "rev-parse", "--is-inside-work-tree")
		if (workTreeExitCode != 0) {
			return Result.Skipped(
				"$repoRoot is not inside a git repository. This is expected in build contexts that " +
					"exclude .git (e.g. the Docker test-runner image; see .dockerignore); not failing " +
					"the build.",
			)
		}
