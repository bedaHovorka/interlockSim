...
**Consequence for Issues #1065/#1076's stale-lock reclamation:** `DefaultPathReservationService.reclaimStaleSwitchLocks` (and, since Issue #1076, `reclaimStaleForeignSwitchOwnership`) releases a switch once its owner holds no BLOCK adjacent to it. That predicate assumes a switch protects only the route(s) it is physically adjacent to. Facade flank grants are the exception — a flank switch protects a route it is NOT adjacent to — so since the Issue #1076 review round the registry marks them: `lockRouteAtomic` registers pure flank switches through `PathReservationRegistry.registerFlankSwitches`, and `PathReservationRegistry.isStaleSwitchOwnership` -- the one staleness rule BOTH reclamation functions use -- never reports a marked switch as stale while its registration stands (the marker is cleared by `unregisterSwitch`/`unregisterSwitches` on release). A future reservation-service flank provisioning (F4) must use the same marker, or extend the reclamation predicates, or both.
