...
| G8 | A route whose END semaphore faces AWAY from the direction of travel at arrival must be rejected before any block, switch or signal is touched (a rear-facing end leaves the train standing at its last facing signal, holding track it cannot use — the #1031/#1060 stall). InOut ends are exempt; the direction is judged per candidate at the target's own cell. Recorded as a geometric failure, so a request whose every candidate ends rear-facing is `GeometricallyImpossible`. | #1064 | `DefaultPathReservationService.rearFacingEndReason` (`reservePath` Step 1.5) |
| G9 | Safety property SI-5: `DynamicRailSwitch.setUpPath` must never move a switch that is locked in a DIFFERENT position — a live route's own switch lock is authoritative, the same way `changeConf` already honours it. A candidate that needs the other position throws `SwitchLockedException`, caught before configuration touches `conf` or `locked`. Unlike G4/G8, this is TRANSIENT contention (`AllPathsBlocked`), never `GeometricallyImpossible` — the switch frees once its holder's route releases it, and misclassifying it would make `InOutWorker` throw instead of letting the train wait (the #1031 stall's root cause: `configureSwitchesInPath` silently re-threw a live route's switch, corrupting it before the Step 2i merge-abort rollback ever ran). A companion reclamation (`DefaultPathReservationService.dropFreedBlock` → `reclaimStaleSwitchLocks`) releases a switch lock once its owner holds no block adjacent to it, so a train that has merely PASSED a switch does not block every other train forever. | #1065 | `DynamicRailSwitch.kt` `setUpPath`; `DefaultPathReservationService.kt` `configureSwitchesInPath`/`SwitchConfigOutcome`/`dropFreedBlock` |
| G10 | Switch ownership is never overwritten: `PathReservationRegistry.registerSwitches` rejects a switch registered to a DIFFERENT train atomically (pre-scan before any mutation), so `switchToTrain` and `trainToSwitches` can never disagree about the owner. A candidate that needs such a switch even in the SAME position is refused as transient `Locked` contention (`AllPathsBlocked`) — but only when the required configuration is known, so a #742 geometric impossibility still classifies as `Unconfigurable`. STALE foreign ownership (owner holds no adjacent block) is reclaimed first in `reservePath` Step 2e.5; facade flank grants (`registerFlankSwitches`) are never stale, since a flank switch protects a route it is not adjacent to (see `INTERLOCKING_SCOPE_LIMITATIONS.md` §B2). | #1076 | `PathReservationRegistry.registerSwitches`/`isStaleSwitchOwnership`; `DefaultPathReservationService.kt` `switchOwnershipPreCheck`/`reclaimStaleForeignSwitchOwnership` |

**Malformation class note:** G4 (rear-facing START rejection), G8 (rear-facing END rejection) and the contiguity precondition (A-R1, [Step 0](#step-0-contiguity-precondition-issue-893-task-a-r1)) are the same malformation class — a route the train cannot actually use (`PathReservationServiceTest.kt:2957`). G9 is explicitly NOT in that class: a G9 refusal is ordinary, resolving contention over a shared switch, not a route the train can never use.
...
| 5 | `rollbackUnconfigurableCandidate` | `:3207` (internal) | Rolls back a candidate whose switches (Step 2f) or START signal (Step 2g) couldn't be configured -- now two switch-configuration causes, Unconfigurable (#742) and Locked (#1065, SI-5), sharing this one rollback. Scoped to the candidate's own mutations (no `registry.unregister(trainId)`); candidate-only switch release delegated to `releaseCandidateSwitches` `:3253` (which releases only switches owned by this train or by NO train -- a switch registered to another train keeps both its lock and its registry entry, Issue #1065 review); per-block `cancelPathSetup` with `registry.unregisterBlock` in `finally` `:3222-3223` (**F3 fix**). Signal-config-failure (G3) reset done by the caller in Step 2g. Neither `releaseCandidateSwitches` nor any other rollback path restores a released switch's *position* (only its lock) -- sound since Issue #1065's G9 guard means the position was never wrongly written in the first place; a rolled-back candidate's own newly-locked-then-unlocked switches carry no authority once unlocked, so their leftover position is harmless. | Caller-enforced |
...
**Switches-last addendum (Issue #1065):** `DefaultPathReservationService.dropFreedBlock` -- the single funnel every block release passes through (`unregisterBlock`'s production call site and `RegistryPartialRouteReleaser`'s tail release both end here) -- now also reclaims a stale switch lock once its owner holds no block adjacent to it (`reclaimStaleSwitchLocks`). This keeps a G9 refusal (above) from turning a train that has merely PASSED a switch into a permanent block for every other train: without it, a switch stayed locked from reservation until the owner's FULL journey completion (`unregister`'s unconditional unlock), confirmed to happen on every repetition of the vyhybna shunting loop. Since Issue #1076 the staleness rule lives in one place (`PathReservationRegistry.isStaleSwitchOwnership`, which also exempts flank-protected switches), shared by this release-time reclamation and the reservation-time `reclaimStaleForeignSwitchOwnership` (Step 2e.5, G10).
