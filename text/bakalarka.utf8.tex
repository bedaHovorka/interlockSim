...
\chapter{Used Languages and Tools}
  ...
  In Java I am used to writing code so that variable and method names are self-commenting.
    ...
    \paragraph{Containers (Collections)} Are analogous to STL from C++ and serve for storing an undefined amount of data. In Java these are objects, thus they themselves can be elements of another collection.
    ...
    Strict static type checking may delay development at first glance,
...
This chapter focuses on a brief description of implemented components. Details are contained in program documentation and source files. First the overall structure of source codes is described and then individual components are elaborated, primarily the most important ones.
  ...
  \section{Simulation}
    ...
    \subsection{Train Movement} \label{trainMoveSection}
      ...
      When the train has completely exited, \sourceRef{Train.Tail} starts. When passing through each node, it
      releases section behind it.

      Each signal provides information about permitted speed behind it and about signal aspect at
      following signal, unless the signal \uv{Stop} is showing. In this case the
      ...
      When passing by a signal it starts changing speed depending on signal aspect.
      So-called "start on caution", when the train at the first signal starts accelerating and before
      the following one stops, is handled separately. Subprocesses end at terminal
      entry-exit points \sourceRef{InOut}. Here I simplified the model by assumption
      that maximum train length must surely be less than minimum possible length of track block between two
      \sourceRef{InOut}. Such similar edge cases are not worth handling otherwise than by declaring the model
      erroneous. Detection of this error requires finding minimal paths.

      Formally it is a combined process (see section \ref{sim:comb}). Discrete part waits for the state condition
      of arrival at a signal and sets acceleration. This interaction describes Petri net in figure \ref{trainProcess},
      it serves however only for illustration. Simulation is not implemented directly using PN.
      I chose simple continuous model, because from the viewpoint of debugging discrete interactions
	...
	\nopagebreak
      ...
      For a better demonstration of how the simulation model works, I created a \uv{shunting loop} controlled based on simple rules.
      ...
      thus from a certain viewpoint represents one indivisible transfer operation.
      ...
      \begin{enumerate}
        ...
        \item goes through all track blocks
	      \begin{enumerate}
	        ...
	        \item if a free path from this signal (previously stored) exists, it sets it
              \end{enumerate}
      \end{enumerate}
      \insertimage{vyhybna}{Track diagram of shunting loop}
      For the diagram in figure \ref{vyhybna}: line segments are tracks, their connections are switches,
...
\end{verbatim}
      ...
      In the graph in figure \ref{shunting} is shown how the train in the model reacts to signal aspects
      during event of arrival at signal.
      Here one can see how the control logic fails to allow path to following block in time,
      and the train accelerates and stops between signals.
      In section \ref{trainMoveSection} the train behavior is explained.

\chapter{Conclusion}
In this chapter, possible program extensions and evaluation of the entire thesis are described.
...
current implementation is directly designed, or which arise from it and are realizable in the near future.

  \subsection*{Goals of the Final Application}
    These goals I set out because I don't want implemented code to be created for only one purpose.
    By this I would like to express possibilities for further development of the entire application.
 ...
 %   genetic algorithms. Not everything however will be possible immediately.
  ...
  \subsection*{Physical Model of Movement}
    For the current movement description, an analytical solution also exists. Description using differential equation
  ...
  \subsection*{Automatic Block}\label{autoblokSection}
     ...
     In figure \ref{autoblok}, the behavior of an AB section is shown.
     \insertimage{autoblok}{C/E Petri net of section in automatic block}
     AB should also be possible to create from set path.
     Furthermore with inter-station sections is also related to the use of other signal types (depends where they will be placed in network):
     \begin{description}
  ...
  \subsection*{Saving Simulation State}
   A discrete process is fragmented into event commands, such as waiting, leading to activity loss. If by some universal way I create possibility to remember
   ...
   The process could be internally represented as an automaton or Petri net and its resp. its state would be
   saved and restored.
   For continuous variables it is necessary to save \sourceRef{jDisco.Variable}.
   It is possible that due to universal solution, it will require modifications in \emph{jDisco} library for serialization support. The state of \sourceRef{LoopProcess} descendants, which inside iteration doesn't come to passive state,
   is possible to save already.
 \subsection*{Visualization Using Cellular Automata, Real-time}
   The square network of the model directly predetermines using cellular automata for rendering control.
   The effort is to minimize the amount of redrawn area and unnecessary computational load,
   caused by traversing states of all cells, by limiting it to only a small number of cells of some types
 ...
 \subsection*{Interface to Model for Dispatcher Control}
   ...
   Create a package of routines and functions covering access to model for control from several levels.
...
The program simulates the movement of several trains in the network simultaneously.
...
Established minimal goals were thus achieved, which example demonstrates. A more complex network is reasonably demonstrable only in case of implementing control module and simulation visualization, when control must be taken over by a human.
The validity of this simulation model could be verified by collecting statistics about real train networks and comparing
with model.

The suitable features of languages resp. tools finally prevail, although some constructs in them were more difficult
to express.
...
The problem I had especially at the beginning with estimation of time complexity of work.
% numerical error
The coding itself went quite quickly, but I forgot about debugging in time estimates.
I prefer to test continuously, errors are then easier to find. For extensive projects however it is still necessary to create
testing scripts.
In the design area there is still what to learn and to better utilize design patterns and mainly adhere to their principles.
However, I must note that time spent studying and designing system was longer than programming time, which I would like in continuation of building this application to further shorten by avoiding mistakes which I committed.
