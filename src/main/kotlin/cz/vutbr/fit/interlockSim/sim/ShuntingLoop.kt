...
class ShuntingLoop : Interlocking {
	...
	private fun getBlock(
		...
		val dynamicBlock = graph.get(point1, point2) ?: throw IllegalArgumentException("Cannot get block between cells")

		// Extract static block from DynamicTrackBlock wrapper (graph contains wrappers after Issue #277)
		val staticBlock = Util.assertInstanceOf(SimpleTrackBlock::class.java, dynamicBlock.staticRef)
		staticBlock.setName(name)  // Set name on underlying static block

		return staticBlock  // Return static block (will be converted to DynamicTrack via toDynamic() during simulation)
	}
