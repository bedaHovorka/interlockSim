 ...
 */
fun <T> Assert<KProperty0<T>>.isEqualTo(expected: T) = given { actual ->
 ...
 */
fun <T> Assert<KProperty0<T>>.isNotEqualTo(expected: T) = given { actual ->
	if (actual.get() != expected) return@given
 ...
 */
fun <T> Assert<KProperty0<T?>>.isNull() = given { actual ->
 ...
 */
fun <T> Assert<KProperty0<T?>>.isNotNull() = given { actual ->
	if (actual.get() != null) return@given
 ...
 * Usage: assertThatCode { /* code */ }.doesNotThrowAnyException()
 */
fun Assert<Result<Unit>>.doesNotThrowAnyException() = given { actual ->
 ...
 */
fun Assert<File>.exists(): Assert<File> = apply {
	given { actual ->
 ...
 */
fun Assert<File>.isFile(): Assert<File> = apply {
	given { actual ->
 ...
 */
fun Assert<String?>.containsAnyOf(vararg substrings: String) = given { actual ->
 ...
 */
fun <R> Assert<Result<R>>.hasMessageContaining(substring: String): Assert<Result<R>> = apply {
	given { actual ->
...
fun Assert<Throwable>.hasMessageContaining(substring: String): Assert<Throwable> = apply {
	given { actual ->
 ...
 */
fun <K, V> Assert<Map<K, V>>.containsEntry(key: K, value: V): Assert<Map<K, V>> = apply {
	given { actual ->
 ...
 * @return The same Assert instance for method chaining
 */
fun <K, V> Assert<Map<K, V>>.doesNotContainValue(value: V): Assert<Map<K, V>> = apply {
	given { actual ->
...
fun <T> Assert<Set<T>>.containsElement(element: T): Assert<Set<T>> = apply {
	given { actual ->
...
fun <T> Assert<Collection<T>>.containsElement(element: T): Assert<Collection<T>> = apply {
	given { actual ->
		if (element in actual) return@given
		expected("to contain:${show(element)} but was:${show(actual)}")
	}
...
fun <T> Assert<Set<T>>.containsExactly(vararg elements: T): Assert<Set<T>> = apply {
	given { actual ->
...
fun <T> Assert<Collection<T>>.containsExactly(vararg elements: T): Assert<Collection<T>> = apply {
	given { actual ->
 ...
 */
fun <T> Assert<Collection<T>>.allMatch(predicate: (T) -> Boolean): Assert<Collection<T>> = apply {
	given { actual ->
 ...
 */
fun <T> Assert<Collection<T>>.isNotEmpty(): Assert<Collection<T>> = apply {
	given { actual ->
...
fun <T> Assert<Collection<T>>.hasSizeChainable(size: Int): Assert<Collection<T>> = apply {
	given { actual ->
...
fun <K, V> Assert<Map<K, V>>.hasSizeChainable(size: Int): Assert<Map<K, V>> = apply {
	given { actual ->
		if (actual.size == size) return@given
		expected("to have size:${show(size)} but was:${show(actual.size)}")
	}
...
fun <T> Assert<Set<T>>.hasSizeChainable(size: Int): Assert<Set<T>> = apply {
	given { actual ->
		if (actual.size == size) return@given
		expected("to have size:${show(size)} but was:${show(actual.size)}")
	}
...
fun <T> Assert<Set<T?>>.containsNull(): Assert<Set<T?>> = apply {
	given { actual ->
...
fun <T> Assert<Collection<T?>>.containsNull(): Assert<Collection<T?>> = apply {
	given { actual ->
		if (actual.any { it == null }) return@given
		expected("to contain null but was:${show(actual)}")
	}
