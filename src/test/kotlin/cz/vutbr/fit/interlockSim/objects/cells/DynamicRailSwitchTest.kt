...
class DynamicRailSwitchTest {
	...
	fun `initial configuration is MAIN`() {
		assertThat(dynamicSwitch1.getConf()).isEqualTo(Conf.MAIN)
	...
	fun `initial lock state is unlocked`() {
		assertThat(dynamicSwitch1.isLocked()).isFalse()
	}
	...
	fun `can change configuration when unlocked`() {
		...
		assertThat(dynamicSwitch1.getConf()).isEqualTo(Conf.BRANCH)
	...
	fun `cannot change configuration when locked`() {
		// Lock the switch
		dynamicSwitch1.lock()
		assertThat(dynamicSwitch1.isLocked()).isTrue()
		
		// Attempt to change configuration
		assertThatThrownBy { dynamicSwitch1.changeConf() }
			.isInstanceOf(IllegalStateException::class.java)
			.hasMessageContaining("Cannot change switch configuration while locked")
			.hasMessageContaining("SI-5")
	}
	...
	fun `configuration state is independent for different dynamic wrappers`() {
		...
		// Verify independence
		assertThat(dynamicSwitch1.getConf()).isEqualTo(Conf.BRANCH)
		assertThat(dynamicSwitch2.getConf()).isEqualTo(Conf.MAIN)
	}
	...
	fun `lock state is independent for different dynamic wrappers`() {
		...
		// Verify independence
		assertThat(dynamicSwitch1.isLocked()).isTrue()
		assertThat(dynamicSwitch2.isLocked()).isFalse()
	}
