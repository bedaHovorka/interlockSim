...
class DynamicRailSemaphoreTest {
	...
	fun `initial signal state is STOP`() {
		assertThat(dynamicSemaphore1.getSignal()).isEqualTo(Signal.STOP)
	}
	...
	fun `can change signal state`() {
		...
		// Change to S30
		dynamicSemaphore1.setSignal(Signal.S30)
		assertThat(dynamicSemaphore1.getSignal()).isEqualTo(Signal.S30)
		
		// Change to FREE
		dynamicSemaphore1.setSignal(Signal.FREE)
		assertThat(dynamicSemaphore1.getSignal()).isEqualTo(Signal.FREE)
	}
	...
	fun `signal state is independent for different dynamic wrappers`() {
		...
		dynamicSemaphore1.setSignal(Signal.S30)
		dynamicSemaphore2.setSignal(Signal.S60)
		
		// Verify independence
		assertThat(dynamicSemaphore1.getSignal()).isEqualTo(Signal.S30)
		assertThat(dynamicSemaphore2.getSignal()).isEqualTo(Signal.S60)
	}
	...
	fun `hashCode is stable across signal state changes`() {
		...
		dynamicSemaphore1.setSignal(Signal.S60)
		assertThat(dynamicSemaphore1.hashCode()).isEqualTo(initialHash)
	...
	fun `equals is stable across signal state changes`() {
		...
		anotherWrapper.setSignal(Signal.FREE)
