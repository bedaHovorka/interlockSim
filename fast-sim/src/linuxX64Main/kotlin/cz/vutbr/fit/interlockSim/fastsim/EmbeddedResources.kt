...
/**
 * XML content embedded directly in the native binary for built-in simulation examples.
 *
 * Strings are inlined (not delegated) so `:core-test` is not on the production classpath.
 * [cz.vutbr.fit.interlockSim.testutil.NetworkResources] holds the canonical copies;
 * [NativeSmokeTest] verifies the two stay in sync.
 *
 * @since Issue #415 (fast-sim native CLI)
 */
internal object EmbeddedResources {

	/** Shunting loop — vyhybna.xml-compatible network. Used by [NativeExampleRegistry]. */
	val VYHYBNA_XML = """
		|<?xml version="1.0"?>
		|<!DOCTYPE net>
		|<net X="100" Y="100">
